...
local function serialize(stream, v, indent)
	...
	elseif t == 'table' then
		...
		for k, kv in next, v do
			local tk = type(k)
			local tkv = type(kv)
			if __SERIALIZABLE[tk] and __SERIALIZABLE[tkv] and (tkv ~= 'table' or next(kv)) and (tk ~= 'table' or next(
				k
			)) and (tk ~= 'string' or k:sub(1, 1) ~= '_') then
...
local loaderEnv = { u = function(class, data)
...
local function loadPersist(hash)
	...
	local loaderFunc, err =
		load('return ' .. contents, '__persister_temp.' .. moduleName .. '.' .. hash, 't', loaderEnv)
